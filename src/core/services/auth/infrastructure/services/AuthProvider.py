...
class AuthProvider(AuthRepository):
    ...
    async def token_rotate(self, request) -> Optional[dict]:
        return await self._token.rotate_tokens(request, self._repo.session, self._db)
    ...
    async def logout(self, request:Request) -> Response:
        ...
        verified_token = int((await self._token.verify_token_unsafe(request, self._token.ACCESS_TYPE)).get('sub'))
    ...
    async def update_profile_user(self, user_id:int,data:dict) -> None:
        await self._repo.update_profile(user_id, data)
    
    @time_checker
    async def password_change(self, email:str, new_password:str):
        await self._repo.change_password_email(email, new_password)
